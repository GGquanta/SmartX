...
Direct Talk text and audio are transient Renderer state. Do not create a SmartX transcript ledger, sidecar, cache, persistence key, direct transcript-file write, synthetic ACP entry, or custom durable timeline projection. OpenClaw and ACP own durable Agent-consult history: a consult uses OpenClaw's existing path and is visible only through normal ACP replay.
