...
Native subagent presentation is independent of attention. Only an exact canonical `agent:<agentId>:subagent:<childId>` key receives the localized subagent tag. SmartX may strip one leading `[Subagent Context]` marker from that row's display title only; it must not infer subagent status from transcript prose or mutate catalog, session, title, or transcript data.
