...
touchedAreas:
  ...
  - resources/openclaw-plugins/smartx-openai-image/index.mjs
  - resources/openclaw-plugins/smartx-openai-image/openclaw.plugin.json
  - resources/openclaw-plugins/smartx-openai-image/package.json
...
expectedUserBehavior:
  ...
  - Saving the OpenAI-compatible image endpoint writes a SmartX-owned provider (`smartx-openai-image`) and auth profile, enables `request.allowPrivateNetwork` for trusted custom endpoints, and leaves `models.providers.openai` untouched so chat continues to use the regular OpenAI provider.
...
`~/.openclaw/agents/{id}/agent/auth-profiles.json`. SmartX's OpenAI-compatible
image endpoint uses a separate `smartx-openai-image` provider/plugin so image
base URL and API key can differ from the normal `openai` chat provider.

SmartX syncs chat defaults on provider switch, but image generation is configured independently from the developer-gated Image Generation tab on Models and is never auto-synced from the default chat provider.
