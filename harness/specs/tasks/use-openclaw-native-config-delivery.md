...
intent: Make one Main-owned coordinator the only SmartX path for OpenClaw config mutation, using config.get and config.set while running and a file fallback before startup.
touchedAreas:
...
expectedUserBehavior:
  ...
  - Every SmartX mutation of the active OpenClaw config goes through one coordinator-owned read-modify-write transaction.
