...
intent: Replace the SmartX-specific Chat stream/history path with ACP session/load, session/prompt, session/cancel, session/update, and session/request_permission while keeping non-Chat Gateway capabilities intact.
touchedAreas:
...
expectedUserBehavior:
  ...
  - The ACP child receives the authoritative SmartX Gateway token through its private process environment, so history replay does not depend on a separately resolved config token.
...
acceptance:
  ...
  - No SmartX ACP replay ledger, Chat history cache, or reduced timeline persistence is introduced.
