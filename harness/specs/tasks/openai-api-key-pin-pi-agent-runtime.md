...
expectedUserBehavior:
  ...
  - Upgrading from an earlier SmartX build that wrote `openai` or `openai-codex` provider entries without `agentRuntime` self-heals those entries on the next provider switch and during pre-launch config sanitization before Gateway reads the config.
  - A pre-existing user-supplied non-empty `agentRuntime.id` is preserved; the SmartX pin only fills missing runtime policy.
requiredProfiles:
  - fast
  - comms
requiredRules:
...
requiredTests:
  - tests/unit/openclaw-auth.test.ts
acceptance:
  - `electron/utils/openclaw-auth.ts` applies a shared OpenAI runtime-pin helper from all SmartX write/self-heal paths and pins `agentRuntime: { id: 'pi' }` on `models.providers.openai` and `models.providers.openai-codex` entries that lack a non-empty runtime id.
...
trajectory, but the shipped SmartX distribution does not register an agent
...
the failure is about agent harness selection. SmartX therefore writes an
