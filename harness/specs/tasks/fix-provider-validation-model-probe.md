...
intent: Prevent custom and plan-specific provider endpoints from rejecting SmartX credential validation because the fallback request uses a fabricated validation-probe model instead of the model configured by the user.
touchedAreas:
...
OpenAI-compatible validation first requests `/models`. When that endpoint is unavailable, SmartX
