...
expectedUserBehavior:
  ...
  - The exact legacy SmartX-managed disabled default is migrated to FTS-only once, after which an explicit user opt-out remains respected.
requiredProfiles:
  - fast
  - comms
requiredTests:
  ...
  - tests/unit/openclaw-auth.test.ts
acceptance:
  - SmartX seeds agents.defaults.memorySearch with enabled true and provider none only when no memory-search configuration and no OpenAI embedding key exist.
...
`agents.defaults.memorySearch.provider: "none"`. Use that mode as SmartX's
