...
intent: Restore OpenClaw image-generation completion replies in SmartX ACP Chat without modifying OpenClaw by projecting trusted ACP and Gateway delivery evidence into the in-memory ACP timeline.
touchedAreas:
...
expectedUserBehavior:
  ...
  - When OpenClaw later exposes a trusted internal-UI source reply through ACP or Gateway host events, SmartX preserves its exact user-facing text instead of replacing it with a generic caption.
...
acceptance:
  - SmartX records recent image_generate background task context from ACP tool output.
  - SmartX accepts only trusted ACP or Gateway completion evidence that matches the active ACP session and recent image-generation context.
  - Internal-UI sourceReply text is authoritative for both successful media replies and text-only failure replies.
  - SmartX hydrates previews through hostApi.media.thumbnails before rendering images.
