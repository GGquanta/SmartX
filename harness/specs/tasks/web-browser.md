...
expectedUserBehavior:
  - SmartX has no standalone Web Browser tab, Home surface, address input, browsing history, site-data controls, or custom guest link menu.
  - The retained guest appears only while Preview displays an authorized local HTML file.
  - Every link is inert; users may only choose to preview the HTML file in SmartX or open that file in the system browser.
requiredProfiles:
  - fast
requiredRules:
