...
title: Disable control-plane agent tools in SmartX
taskType: runtime-bridge
intent: Prevent SmartX agents from invoking Gateway, node, and goal-management control-plane tools while preserving application-owned Gateway RPCs and agent orchestration tools.
...
expectedUserBehavior:
  - SmartX agents cannot invoke gateway, nodes, create_goal, get_goal, or update_goal.
  ...
  - SmartX application-owned Gateway RPCs continue to work because tool policy does not block internal RPC methods.
requiredProfiles:
  - fast
  - comms
requiredTests:
...
acceptance:
  ...
  - Existing SmartX-required deny entries remain enforced.
  - Targeted unit tests, type checks, communication regression checks, and harness validation pass.
docs:
  required: true
---

SmartX keeps application-owned Gateway communication and agent orchestration available while preventing models from invoking selected control-plane tools directly. The restriction is enforced at both OpenClaw tool-policy layers.
