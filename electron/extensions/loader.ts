...
function resolveManifestPath(): string {
  if (app.isPackaged) {
    return join(process.resourcesPath, 'smartx-extensions.json');
  }
  return join(app.getAppPath(), 'smartx-extensions.json');
}
...
export async function loadExtensionsFromManifest(): Promise<void> {
  ...
  if (existsSync(manifestPath)) {
    ...
    logger.debug('[extensions] No smartx-extensions.json found, loading all builtin extensions');
  }
