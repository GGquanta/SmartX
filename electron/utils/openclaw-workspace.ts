...
const SMARTX_BEGIN = '<!-- smartx:begin -->';
const SMARTX_END = '<!-- smartx:end -->';
...
/**
 * Ensure SmartX-managed workspaces have a non-template IDENTITY.md before the
 * Gateway initializes them. Existing custom identities are preserved.
 */
export async function ensureSmartXIdentityFile(
  workspaceDir: string,
  ...
  const defaultIdentity = buildDefaultSmartXIdentityContent();
  ...
  if (await fileExists(bootstrapPath)) {
    try {
      ...
      logger.info(`Removed chat-first bootstrap file from SmartX workspace (${resolvedWorkspaceDir})`);
    } catch {
      logger.warn(`Failed to remove chat-first bootstrap file: ${bootstrapPath}`);
    }
  } else if (wroteIdentity) {
    logger.info(`Seeded default SmartX identity for workspace (${resolvedWorkspaceDir})`);
  }
}

export async function ensureSmartXDefaultIdentity(): Promise<void> {
  const workspaceDirs = await resolveAllWorkspaceDirs();
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    await ensureSmartXIdentityFile(workspaceDir, { createDir: waitForGatewaySeed });
  }
}
...
/**
 * Merge a SmartX context section into an existing file's content.
 * If markers already exist, replaces the section in-place.
 * Otherwise appends it at the end.
 */
export function mergeSmartXSection(existing: string, section: string): string {
  const wrapped = `${SMARTX_BEGIN}\n${section.trim()}\n${SMARTX_END}`;
  const beginIdx = existing.indexOf(SMARTX_BEGIN);
  const endIdx = existing.indexOf(SMARTX_END);
  if (beginIdx !== -1 && endIdx !== -1) {
    return existing.slice(0, beginIdx) + wrapped + existing.slice(endIdx + SMARTX_END.length);
  }
 ...
 * for SmartX-managed workspaces.  Removes everything from the heading
...
/**
 * Detect and remove bootstrap .md files that contain only SmartX markers
 * with no meaningful OpenClaw content outside them.
 */
export async function repairSmartXOnlyBootstrapFiles(): Promise<void> {
  const workspaceDirs = await resolveAllWorkspaceDirs();
  for (const { dir: workspaceDir } of workspaceDirs) {
    ...
    for (const file of entries) {
      ...
      const beginIdx = content.indexOf(SMARTX_BEGIN);
      const endIdx = content.indexOf(SMARTX_END);
      if (beginIdx === -1 || endIdx === -1) continue;

      const before = content.slice(0, beginIdx).trim();
      const after = content.slice(endIdx + SMARTX_END.length).trim();
      if (before === '' && after === '') {
        try {
          await unlink(filePath);
          logger.info(`Removed SmartX-only bootstrap file for re-seeding: ${file} (${workspaceDir})`);
        } catch {
          logger.warn(`Failed to remove SmartX-only bootstrap file: ${filePath}`);
        }
      }
    }
  }
...
/**
 * Merge SmartX context snippets into workspace bootstrap files that already
...
type EnsureSmartXContextOptions = {
  /**
...
async function mergeSmartXContextOnce(options: EnsureSmartXContextOptions = {}): Promise<MergeResult> {
  const contextDir = join(getResourcesDir(), 'context');
  if (!(await fileExists(contextDir))) {
    logger.debug('SmartX context directory not found, skipping context merge');
    return { missing: 0, retryableMissing: 0 };
  }

  let files: string[];
  try {
    files = (await readdir(contextDir)).filter((f) => f.endsWith('.smartx.md'));
  } catch {
    return { missing: 0, retryableMissing: 0 };
  }
  ...
  for (const { dir: workspaceDir, waitForGatewaySeed } of workspaceDirs) {
    ...
    for (const file of files) {
      const targetName = file.replace('.smartx.md', '.md');
      ...
      const merged = mergeSmartXSection(existing, section);
      // Compare against on-disk content so we persist changes even when only
      // First Run stripping happened and the SmartX section stayed identical.
      if (merged !== originalExisting) {
        await writeFile(targetPath, merged, 'utf-8');
        logger.info(`Merged SmartX context into ${targetName} (${workspaceDir})`);
      }
    }
  }
...
const MAX_RETRIES = 5;
let ensureSmartXContextPromise: Promise<void> | null = null;
let ensureSmartXContextWaitsForAll = false;

/**
 * Ensure SmartX context snippets are merged into the openclaw workspace
 * bootstrap files.
 */
export async function ensureSmartXContext(options: EnsureSmartXContextOptions = {}): Promise<void> {
  if (ensureSmartXContextPromise) {
    if (options.waitForAllConfiguredWorkspaces && !ensureSmartXContextWaitsForAll) {
      return ensureSmartXContextPromise.then(() => ensureSmartXContext(options));
    }
    return ensureSmartXContextPromise;
  }

  ensureSmartXContextWaitsForAll = options.waitForAllConfiguredWorkspaces === true;
  ensureSmartXContextPromise = runEnsureSmartXContext(options).finally(() => {
    ensureSmartXContextPromise = null;
    ensureSmartXContextWaitsForAll = false;
  });
  return ensureSmartXContextPromise;
}

async function runEnsureSmartXContext(options: EnsureSmartXContextOptions): Promise<void> {
  let result = await mergeSmartXContextOnce(options);
  if (result.retryableMissing === 0) {
    if (result.missing > 0) {
      logger.debug(`SmartX context merge skipped ${result.missing} non-ready file(s)`);
    }
    return;
  }
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    result = await mergeSmartXContextOnce(options);
    if (result.retryableMissing === 0) {
      logger.info(`SmartX context merge completed after ${attempt} retry(ies)`);
      return;
    }
    logger.debug(`SmartX context merge: ${result.retryableMissing} startup file(s) still missing (retry ${attempt}/${MAX_RETRIES})`);
  }

  logger.warn(`SmartX context merge: ${result.retryableMissing} startup file(s) still missing after ${MAX_RETRIES} retries`);
}
