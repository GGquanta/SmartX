...
interface SyncProxyOptions {
   ...
   * currently disabled in SmartX settings.
   */
  preserveExistingWhenDisabled?: boolean;
}

/**
 * Sync SmartX global proxy settings into OpenClaw channel config where the
...
export async function syncProxyConfigToOpenClaw(
  ...
  return withConfigLock(async () => {
    ...
    if (!settings.proxyEnabled && preserveExistingWhenDisabled && currentProxy) {
      logger.info('Skipped Telegram proxy sync because SmartX proxy is disabled and preserve mode is enabled');
      return;
    }
