...
const PATCH_MARKER = 'SmartX-patched-v2: extract directly to $INSTDIR and fail closed';
const LEGACY_PATCH_MARKER = 'SmartX-patched: extract directly to $INSTDIR';
const LEGACY_CONTINUE_ON_EXTRACT_FAILURE = 'continuing overwrite install anyway';
const FATAL_EXTRACT_FAILURE_DETAIL = 'Failed to extract SmartX files after multiple attempts.';
const ROLLBACK_EXTRACT_FAILURE_DETAIL = 'Restoring previous SmartX installation after failed update';

const PATCHED_EXTRACT_MACRO = [
  '!macro extractUsing7za FILE',
  `  ; ${PATCH_MARKER}.`,
  '  StrCpy $R9 0',
  '  smartx_extract_attempt:',
  '    IntOp $R9 $R9 + 1',
  '    DetailPrint "Extracting SmartX application files (attempt $R9, please wait)..."',
  '    SetOutPath $INSTDIR',
  '    ClearErrors',
  '    Nsis7z::Extract "${FILE}"',
  '    IfErrors 0 smartx_extract_done',
  '    ${if} $R9 < 5',
  ...
  '      Pop $1',
  '      Sleep 3000',
  '      Goto smartx_extract_attempt',
  '    ${endIf}',
  '    DetailPrint "Failed to extract SmartX files after multiple attempts."',
  '    ${if} $smartxRollbackDir != ""',
  '      IfFileExists "$smartxRollbackDir\\" 0 smartx_extract_show_error',
  '      DetailPrint "Restoring previous SmartX installation after failed update..."',
  '      SetOutPath $TEMP',
  '      RMDir /r "$INSTDIR"',
  '      Rename "$smartxRollbackDir" "$INSTDIR"',
  '    ${endIf}',
  '  smartx_extract_show_error:',
...
export function patchNsisExtractTemplate(targetPath = EXTRACT_APP_PACKAGE_NSH) {
  ...
  if (hasStaleExtractPatch(original)) {
    console.warn('[patch-nsis-extract] Stale SmartX extract patch detected; replacing with fail-closed patch.');
