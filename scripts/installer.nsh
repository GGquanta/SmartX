...
Function SmartXMoveLegacyInstallDir
  Exch $R6

  ${if} $R6 == ""
    Goto _smartx_legacy_move_done
  ${endIf}
  ${if} $R6 == $INSTDIR
    Goto _smartx_legacy_move_done
  ${endIf}

  IfFileExists "$R6\" 0 _smartx_legacy_move_done
    DetailPrint "Moving previous SmartX installation at $R6 out of the way..."
    SetOutPath $TEMP
  ...
  _smartx_legacy_find_free_stale:
    IfFileExists "$R6._stale_$R8\" 0 _smartx_legacy_found_free_stale
    IntOp $R8 $R8 + 1
    Goto _smartx_legacy_find_free_stale

  _smartx_legacy_found_free_stale:
    ClearErrors
    Rename "$R6" "$R6._stale_$R8"
    IfErrors 0 _smartx_legacy_stale_moved
      ...
      Rename "$R6" "$R6._stale_$R8"
      IfErrors 0 _smartx_legacy_stale_moved
      DetailPrint "Removing previous SmartX installation at $R6..."
      nsExec::ExecToStack 'cmd.exe /c rd /s /q "$R6"'
      Pop $0
      Pop $1
      Goto _smartx_legacy_move_done
  _smartx_legacy_stale_moved:
    ExecShell "" "cmd.exe" `/c ping -n 61 127.0.0.1 >nul & rd /s /q "$R6._stale_$R8"` SW_HIDE

  _smartx_legacy_move_done:
    ClearErrors
    Pop $R6
FunctionEnd

!macro smartxMoveLegacyInstallDir ROOT_KEY
  ReadRegStr $R6 ${ROOT_KEY} "${INSTALL_REGISTRY_KEY}" InstallLocation
  Push $R6
  Call SmartXMoveLegacyInstallDir
!macroend
!endif
...
!macro customCheckAppRunning
  ...
  ; commands instead of nsProcess here: field diagnostics showed SmartX.exe can
  ...
  _smartx_verify_closed:
    ...
    ${if} $R0 == 0
      ...
      Sleep 2000
      ${if} $R7 < 5
        Goto _smartx_verify_closed
      ${endIf}
      MessageBox MB_RETRYCANCEL|MB_ICONEXCLAMATION "SmartX is still running and cannot be replaced safely. Please close SmartX and retry installation." /SD IDCANCEL IDRETRY _smartx_verify_closed
      SetErrorLevel 2
      Quit
    ${endIf}

  !ifndef BUILD_UNINSTALLER
    StrCpy $smartxRollbackDir ""
    ...
    !insertmacro smartxMoveLegacyInstallDir HKCU
    !insertmacro smartxMoveLegacyInstallDir HKLM
  ...
  _found_free_stale:
    ...
    IfErrors 0 _stale_moved
      ...
      IfFileExists "$INSTDIR\" 0 _recreate_clean_instdir
        ...
        MessageBox MB_OK|MB_ICONEXCLAMATION "Unable to replace the previous SmartX installation because files are still locked. Please close SmartX and retry installation." /SD IDOK
        SetErrorLevel 2
        Quit
      ...
      Goto _instdir_clean
  _stale_moved:
    StrCpy $smartxRollbackDir "$INSTDIR._stale_$R8"
    CreateDirectory "$INSTDIR"
  _instdir_clean:
...
!macro customInstall
  ...
  DetailPrint "Clearing stale SmartX registry entries from the opposite install scope..."
  ${if} $installMode == "all"
