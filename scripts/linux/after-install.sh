...
# Post-installation script for SmartX on Linux
...
# Create symbolic link for SmartX app binary
if [ -x /opt/SmartX/smartx ]; then
    ln -sf /opt/SmartX/smartx /usr/local/bin/smartx 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/SmartX/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/SmartX/chrome-sandbox' || true
else
    chmod 0755 '/opt/SmartX/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/SmartX/resources/apparmor-profile'
...
echo "SmartX has been installed successfully."
