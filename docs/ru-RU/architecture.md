...
SmartX использует **двухпроцессную архитектуру с унифицированным уровнем Host API**. Renderer обращается к единой абстракции клиента, а Electron Main управляет выбором протокола и жизненным циклом процессов.

Доставка конфигурации OpenClaw также управляется Electron Main. Когда Gateway запущен, SmartX использует авторитетный снимок из `config.get` как основу и применяет изменения через `config.set`. Когда Gateway остановлен или запускается, тот же координатор обновляет разрешённый JSON5-файл конфигурации, не запуская Gateway из-за этого обновления. Поэтому обычные изменения провайдера, агента, канала, привязки, навыка и модели не заменяют процесс Gateway. Полные перезапуски остаются только для изменений среды запуска процесса, например прокси, и явных действий пользователя. Подтверждённые завершения процесса и закрытия WebSocket используют существующие пути автоматического переподключения. Первые три последовательных пропуска WebSocket heartbeat являются только диагностикой, поэтому краткая задержка pong не прерывает долгую операцию; pong или любое входящее сообщение сбрасывает счётчик, а при четвёртом последовательном пропуске запрашивается защищённое автоматическое восстановление Gateway, если его жизненный цикл находится в состоянии running с разрешённым автовосстановлением. После записи конфигурации аутентификации в SQLite SmartX вызывает `secrets.reload` OpenClaw, чтобы работающие агенты получили новые учётные данные без перезапуска процесса.

Chat использует ACP stdio bridge, принадлежащий Electron Main. Main передаёт тому же локальному дочернему процессу управляемый приложением Gateway token через приватное окружение процесса, поэтому после перезагрузки конфигурации среды выполнения воспроизведение истории ACP остаётся аутентифицированным. Если защищённое восстановление Gateway прерывает принятый run основной сессии, исправленная среда OpenClaw запускает отдельный восстановительный run с явной ссылкой на id непосредственно прерванного run. События Chat и agent сохраняют эту связь; переподключившийся ACP bridge последовательно принимает каждый новый run для ожидающего prompt, сбрасывает потоковые курсоры run и подписывается на события инструментов сессии. Если последующий перезапуск теряет внутрипроцессное терминальное уведомление после сохранения ответа, ACP передаёт текущие run id и session key в `agent.wait`; Gateway завершает ожидание только при совпадении сохранённого lifecycle owner с этим run. Renderer не использует идентификатор среды Gateway и продолжает строить одну ACP timeline в памяти из типизированных host events. Gateway продолжает отвечать за возможности вне Chat: providers, models, skills, workspace, settings, diagnostics и media configuration.
...
Исправленный OpenClaw восстанавливает допустимый размер prompt до отправки Provider. Если совокупный текст результатов инструментов превышает бюджет prompt после вычета резерва, среда вычисляет единую цель усечения из измеренного превышения и защитного буфера и повторно использует её при восстановлении mid-turn, pre-prompt и post-compaction. Сначала сокращаются старые результаты, при этом сохраняются пары вызов/результат инструмента и ограниченное представление новейших результатов. Ответ сжатия об отсутствии реальных сообщений беседы не отменяет измеренное давление transcript или сформированного prompt. В структурированных событиях ошибки сжатия источник запуска отделён от необязательного стабильного кода причины, а текст причины ограничивается 500 символами до записи в ACP.

### Авторитет истории ACP и ограниченные дополнения из transcript

Воспроизведение ACP `session/load` является главным источником истории Chat. SmartX не сохраняет второй ACP ledger, сокращённую timeline, кэш воспроизведения или восстановленную историю инструментов. Если структурированный ACP event ledger OpenClaw недоступен, его ACP adapter преобразует сохранённые записи transcript `toolCall` и `toolResult` в нативные обновления инструментов в исходном порядке, сохраняя границы text-tool-text; сам SmartX эти записи не выводит. Некоторые возможности OpenClaw пока не имеют полного соответствия в ACP. Например, media assistant может отсутствовать в ACP, а обработка Gateway может удалять директивы assistant `MEDIA:` из видимого потокового ответа. Поэтому SmartX хранит только ограниченные, помеченные дополнения совместимости в памяти:

#### Replay hydration завершённого хода

Пока ACP prompt выполняется, уведомления `session/update` немедленно обновляют видимую timeline. Renderer ожидает завершения типизированного вызова `session/prompt` и только после его успешного ответа сразу выполняет один `session/load` для той же session; фиксированной паузы между операциями нет. Hydration начинается только если session, generation, workspace и Renderer load request всё ещё актуальны. Main сериализует load на общем ACP connection, выделяет следующую routing generation, буферизует replay notifications до завершения upstream `session/load` и возвращает raw batch вместе с результатом load.

Во время load Renderer сохраняет завершённую live timeline и не показывает пустое loading-состояние. Events следующей generation, пришедшие во время передачи IPC-результата, также буферизуются. Успешный непустой batch фильтруется по возвращённым session и generation, сворачивается обычным reducer начиная с пустой ACP timeline, после чего полная timeline и generation атомарно публикуются одним обновлением state. Pending attachments повторно разрешаются для новой generation, live turn timing переносится на identity пользовательского сообщения из replay, а replay-доказательство изображения замещает незавершённую проекцию старой generation. Ошибка, исключение, stale или superseded hydration не заменяют live content. Успешный пустой результат или `resumed-active-prompt` сохраняет видимые items, но принимает уже зафиксированную Main generation, чтобы не потерять последующие events.

Успешное завершение `session/prompt` служит причинным барьером settlement. Безусловная задержка лишь увеличивает latency, дольше сохраняет sending-состояние и расширяет окно, в котором navigation или другой load отменяет актуальность hydration, но не доказывает завершение upstream-персистентности. Если будущая upstream-реализация действительно вернёт prompt success до готовности durable replay, следует использовать ограниченный условный retry для пустого replay или отсутствующего текущего хода с теми же identity guards, а не фиксированную задержку. Указанный ниже retry через 1500 мс относится только к ограниченным transcript-дополнениям и не является частью ACP replay hydration.
...
- Полное ACP replay может восстановить записанные файловые операции. При неполном replay SmartX не выводит пропущенную активность через fallback.

```
┌──────────────────────────────────────────────────────────────────┐
│                        Десктопное приложение SmartX                │
│                                                                  │
│  ┌────────────────────────────────────────────────────────────┐  │
...
| Перезапускать только недоступный процесс под управлением SmartX | При неудачном deadline probe запрашивать защищённый путь перезапуска Gateway | Восстанавливать действительно не отвечающий локальный дочерний процесс |
| Никогда не останавливать внешний Gateway автоматически | Предпочтительно заменять или переподключать только WebSocket SmartX и сообщать о недоступности в диагностике | Не отправлять shutdown процессу, которым SmartX не владеет |
...
- SmartX — приложение Electron, поэтому **один экземпляр обычно отображается как несколько процессов ОС** (main/renderer/zygote/utility). Это нормально.
...
- Нажатие кнопки закрытия окна (`X`) скрывает SmartX в трее, но не завершает приложение. Для полного завершения используйте **Quit SmartX** в меню трея.
