...
Когда активная ACP-сессия OpenClaw сообщает об использовании контекста, в нижней строке состояния композитора сразу слева от статуса подключения шлюза отображается компактный индикатор с кольцом и видимым локализованным процентом. Наведите на него курсор или сфокусируйте его, чтобы увидеть количество использованных/всех токенов. Количество использованных и всех токенов обычно берётся из активного ACP `usage_update`. После переключения модели эффективное окно контекста из обновлённого снимка Agent заменяет устаревший общий лимит до следующего отчёта ACP, поэтому лимит и процент обновляются сразу. Индикатор скрыт при отсутствии или недействительности данных об использовании.

В правом верхнем углу чата можно показать или скрыть ход мыслей и вызовы инструментов. По умолчанию они видны; при отключении на шкале остаются ответы, планы, разрешения и активность файлов.
Упорядоченная временная шкала процессов ACP также показывает сжатие контекста. Маркер активного сжатия обновляется на месте: с выполнения на завершение, продолжение после сжатия, ошибку или отмену. Если OpenClaw передаёт причину ошибки, маркер ошибки показывает локализованное ограниченное пояснение. Завершённые исторические маркеры восстанавливаются по порядку только для записей сжатия, вошедших в ограниченный ответ истории ACP от OpenClaw. Более старые записи за пределами этого ответа не загружаются в это представление, а сводка сжатия в маркере никогда не показывается. В насыщенных инструментами ходах совокупное давление prompt устраняется сокращением более старых результатов до измеренного бюджета с сохранением ограниченного представления новейших результатов и их связи с вызовами инструментов.

При выборе другого агента через `@agent` SmartX напрямую переключается в контекст этого агента, не передавая запрос через агента по умолчанию. Рабочие пространства агентов по умолчанию разделены, а более строгая изоляция среды выполнения зависит от настроек sandbox OpenClaw.
...
Вкладки Workspace и Preview на правой панели Chat предоставляют предпросмотр только для чтения файлов Markdown, `.docx` и `.pptx`. Предпросмотр Markdown в статическом режиме использует ту же подсветку синтаксиса, перенос длинных строк, копирование fenced code, разбор CJK и поддержку формул KaTeX. В заголовке Preview можно развернуть выбранный файл на весь видимый экран SmartX; тем же элементом управления или клавишей Escape можно вернуться на панель. Устаревшие `.doc` и `.ppt` по-прежнему открываются через операционную систему, а не внутри приложения. Разбиение DOCX на страницы может отличаться от Microsoft Word; предпросмотр PPTX не поддерживает анимации, переходы и воспроизведение медиа. Office-файлы размером более 20 МБ не просматриваются внутри приложения.
...
Для пользовательских ID аккаунтов каналов SmartX требует совместимый с OpenClaw канонический формат: `[a-z0-9_-]`, строчные буквы, максимум 64 символа, начало с буквы или цифры. Это предотвращает ошибки маршрутизации.

SmartX также включает официальный плагин личного WeChat от Tencent, поэтому WeChat можно подключить прямо на странице Channels через встроенный QR-код.
...
SmartX предварительно упаковывает полные навыки обработки документов (`pdf`, `xlsx`, `docx`, `pptx`), автоматически развёртывает их в управляемый каталог навыков (по умолчанию `~/.openclaw/skills`) при запуске и включает их по умолчанию при первой установке.
...
В режиме разработчика откройте вкладку **Models -> Image Generation**, чтобы настроить независимый OpenAI-совместимый эндпоинт генерации изображений с Base URL, API-ключом и именем модели, например `gpt-image-2`. Поэтому генерация изображений может использовать отдельный сервис `/v1/images/generations`, а чат продолжает использовать обычный OpenAI-провайдер.
...
При редактировании или переключении провайдера SmartX сохраняет существующие метаданные возможностей модели, например `input: ["text", "image"]`. Для новых моделей Custom-провайдера используется совместимая с OpenClaw onboarding логика определения поддержки изображений; неизвестные модели считаются текстовыми.

SmartX сохраняет явно заданные `contextWindow` или `contextTokens` в строках моделей провайдера, но не определяет отсутствующий предел контекста по имени модели. Резерв сжатия равен 25% только при наличии явного действительного предела; иначе `reserveTokensFloor` получает консервативное значение 50000 токенов. Если настройки сжатия отсутствуют, SmartX также создаёт `agents.defaults.compaction.mode = "safeguard"`, `keepRecentTokens = 0`, `recentTurnsPreserve = 0` и `midTurnPrecheck.enabled = true`. Синхронизация при запуске всегда устанавливает оба параметра сохранения истории в `0`, чтобы все завершённые ходы попадали в сводку, а не воспроизводились дословно после сжатия; явный выбор `midTurnPrecheck.enabled` сохраняется.
...
Если совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, SmartX во время проверки API-ключа автоматически переключается на лёгкий зонд `/chat/completions` или `/responses` с настроенной моделью.

### Адаптивные темы

Выбирайте светлую, тёмную или синхронизированную с системой тему. SmartX автоматически адаптируется к вашим предпочтениям.

### Управление автозапуском

В разделе **Настройки → Общие** включите **Запускать при старте системы**, чтобы SmartX автоматически запускался после входа в систему.

### Запросы на обновление

SmartX проверяет наличие новых версий при запуске. Если обновление доступно, приложение показывает запрос; скачивание и установка выполняются только после вашего выбора.
