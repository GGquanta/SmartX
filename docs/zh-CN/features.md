...
当活动的 OpenClaw ACP 会话上报上下文用量时，输入框下方的状态栏会在网关连接状态左侧显示一个带用量环和本地化百分比的紧凑进度指示器。悬停或聚焦该指示器可查看已用/总 token 数。已用和总 token 通常来自活动 ACP 的 `usage_update`；切换模型后，在 ACP 上报新用量前，更新后的 Agent 快照中的有效上下文窗口会替换旧总量，因此上限和百分比会立即更新；用量元数据缺失或无效时不会显示。
对话页右上角可切换是否显示思考过程和工具调用过程；默认显示，关闭后时间线仍保留回复、计划、权限和文件活动。
ACP 有序流程时间线也会显示上下文压缩状态。实时标记会在原位置从“正在压缩”更新为“已完成”“已压缩并继续”“失败”或“已取消”；OpenClaw 提供失败原因时，失败标记会显示本地化的有界原因说明。历史中的已完成标记则按顺序从 OpenClaw 有界 ACP 历史响应所包含的压缩记录恢复。此视图不会加载该响应之外更早的记录，标记也绝不会显示压缩摘要。工具密集型回合遇到聚合 prompt 压力时，会按实测预算优先截断较早的工具输出，同时保留最新结果的有界表示及其工具调用配对。
...
Chat 右侧面板的工作空间和预览选项卡支持以只读方式预览 Markdown、`.docx` 和 `.pptx` 文件。Markdown 文件预览以静态渲染模式提供相同的围栏代码语法高亮、软换行与复制操作、面向中日韩文本的解析和 KaTeX 数学公式支持。预览栏顶部可将当前文件展开至 SmartX 的整个可视区域；再次点击该按钮或按 Esc 即可返回侧栏。旧版 `.doc` 和 `.ppt` 文件不会在应用内预览，而是继续通过操作系统打开。DOCX 的分页效果可能与 Microsoft Word 不同；PPTX 预览不支持动画、切换效果或媒体播放。超过 20 MB 的 Office 文件不会在应用内预览。

### 本地 HTML 预览
Chat 右侧面板只包含工作空间、预览和变更，不再提供通用网页浏览器、主页或地址栏。已授权的本地 `.html` 和 `.htm` 附件、文件活动及工作空间文件默认在预览中打开。文件操作可以选择 SmartX 内置预览或系统应用，预览标题栏也可将当前 HTML 文件交给系统浏览器打开。

所有链接都不可点击。SmartX 渲染的链接显示为普通文本，HTML 预览中的链接也会移除链接样式和指针交互。HTML 预览同时阻止表单、脚本跳转、重定向、页内跳转、弹窗、下载、网络请求和设备权限；它可以显示自包含的本地 HTML，但无法离开当前选中的文档。
...
SmartX 还会内置预装完整的文档处理技能（`pdf`、`xlsx`、`docx`、`pptx`），在启动时自动部署到托管技能目录（默认 `~/.openclaw/skills`），并在首次安装时默认启用。
...
SmartX 会保留 Provider 模型行中显式设置的 `contextWindow` 或 `contextTokens`，但不会根据模型名称推断缺失的上下文上限。上下文压缩预留仅在存在显式有效上限时按其 25% 计算；未设置时，`reserveTokensFloor` 使用保守的 50000 token 默认值。当你没有配置 compaction 时，SmartX 还会默认写入 `agents.defaults.compaction.mode = "safeguard"`、`keepRecentTokens = 0`、`recentTurnsPreserve = 0` 和 `midTurnPrecheck.enabled = true`。启动同步始终会将这两个历史保留值设为 `0`，使所有已完成回合进入摘要，而不是在压缩后逐字重放；显式设置的 `midTurnPrecheck.enabled` 仍沿用现有策略。
...
如果兼容网关的 `/models` 因非鉴权原因不可用，SmartX 会在校验 API Key 时使用已配置的模型，自动降级为轻量的 `/chat/completions` 或 `/responses` 探测。
